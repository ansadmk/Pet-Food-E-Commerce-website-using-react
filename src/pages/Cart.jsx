...
function Cart() {
  ...
  return (
    <div>
      ...
      <Row className="container m-auto w-75 overflow-auto" style={{maxHeight:"500px"}}>
        {state.map((a, i) =>
          a.id == cur ? <CartMap value={a.userProduct} index={i} /> : null
        )}{" "}
        <div></div>{" "}
      </Row>
      <h1 className="text-center bg-dark-subtle container mt-5 rounded-4 p-2"><Comptotal /></h1>
    </div>
  );
}
